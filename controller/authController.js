...
const authController = {
        ...
        return accountActivationMail(email, name, activationKey).then(() => {
          ...
          return newUser.save((error) => {
            ...
            return res.status(200).send({
              status: true,
              message: 'Activation mail has been sent. Please check your email and activate your account.'
            });
          });
  ...
  login(req, res) {
    ...
    const {
      name,
      ext
    } = req.user.profileImageName ? splitFileName(req.user.profileImageName) : {};
